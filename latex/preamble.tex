...
% Dipnot ayraç çizgisi: varsayılan LaTeX kısa (0.4\columnwidth) çiziyor, tam sayfa
% genişliğine (\textwidth) çıkarıldı.
...
\usepackage{graphicx}
\graphicspath{{gorseller/bolum01-sayi-sistemleri/}{gorseller/bolum02-bool-cebri/}{gorseller/bolum03-kmap/}}
\usepackage{float}
...
\lstloadlanguages{[ANSI]C, C++, Java}  % listings 1.11b uyumsuzluk düzeltmesi: dilleri önceden yükle
% Kod dosyalarının tek doğru kaynağı repo kökündeki kod/ klasörü (GitHub'da ayrıca
% taranabilir/indirilebilir); \lstinputlisting bunlardan doğrudan okur, kitapta
% metni elle kopyalamak yerine.
\lstset{
    inputpath=../kod/,
...
\lstdefinestyle{python}{
    morekeywords={def, return, for, while, if, elif, else, in, not, and, or, break, continue, import, from, as, class, lambda, with, yield, None, True, False},
    keywordstyle=\color{blue}\bfseries,
    commentstyle=\color{green!50!black}\ttfamily,
    stringstyle=\color{red},
    morecomment=[l]{\#},
...
% --- Program çıktısı (konsol) blokları için stil ---
\lstdefinestyle{cikti}{
    numbers=none,
    xleftmargin=1em,
    framexleftmargin=0.5em,
    frame=single,
    backgroundcolor=\color{acikgri!50},
...
% Örnek kutusu — numaralı (Bölüm.Sıra biçiminde, ör. Örnek 1.1, Örnek 1.2, ...)
\newcounter{ornek}[chapter]
\renewcommand{\theornek}{\thechapter.\arabic{ornek}}
\newtcolorbox{orneknum}[1]{
    colback=bolumrenk!5,
    colframe=bolumrenk,
    fonttitle=\bfseries,
    title={#1},
...
\newenvironment{ornek}[1][]{%
    \refstepcounter{ornek}%
    \begin{orneknum}{Örnek~\theornek: #1}%
}{%
    \end{orneknum}%
}
...
% Alıştırma zorluk dereceleri (yıldız glifi Libertinus'ta yok, \symbolfont ile alınır)
\newcommand{\kolay}{{\normalfont\small\textcolor{green!60!black}{\symbolfont★}}}
\newcommand{\orta}{{\normalfont\small\textcolor{orange!80!black}{\symbolfont★★}}}
\newcommand{\zor}{{\normalfont\small\textcolor{red!70!black}{\symbolfont★★★}}}
...
% Detaylı Bilgi sayacı ve listesi (numaralı, terimrenk/kahverengi kutu)
\newcounter{detaynotu}[chapter]
\renewcommand{\thedetaynotu}{\thechapter.\arabic{detaynotu}}

\makeatletter
\newcommand{\listofdetaybilgiler}{%
    \chapter*{Detaylı Bilgi Listesi}%
    \addcontentsline{toc}{chapter}{Detaylı Bilgi Listesi}%
    \@starttoc{lod}%
}
\newcommand{\l@detaynotu}{\@dottedtocline{1}{0em}{3em}}
\makeatother

% Detaylı bilgi iç kutusu (doğrudan kullanılmaz)
\newtcolorbox{detaybilgiic}[1]{
    colback=terimrenk!5!white,
    colframe=terimrenk!80!black,
    fonttitle=\bfseries,
    title={#1},
...
% Detaylı bilgi ortamı (numaralı, listeye eklenir)
\newenvironment{detaybilgi}[1][]{%
    \refstepcounter{detaynotu}%
    \def\detaybilgiarg{#1}%
    \ifx\detaybilgiarg\empty
        \addcontentsline{lod}{detaynotu}{\protect\numberline{\thedetaynotu}Detaylı Bilgi}%
        \begin{detaybilgiic}{Detaylı Bilgi~\thedetaynotu}%
    \else
        \addcontentsline{lod}{detaynotu}{\protect\numberline{\thedetaynotu}#1}%
        \begin{detaybilgiic}{Detaylı Bilgi~\thedetaynotu: #1}%
    \fi
}{%
    \end{detaybilgiic}%
}
