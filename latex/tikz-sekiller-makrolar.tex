...
\definecolor{sinyallacivert}{RGB}{30, 60, 130}   % yerleşik (artık yeni olmayan) denetim sinyalleri
...
\newcommand{\karnoiki}[4][]{%
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \node[above left] at (0, 2) {\footnotesize $\boldsymbol{#2\backslash #3}$};
        % Hücre değerleri
...
\newcommand{\karnodort}[4][]{%
    \begin{tikzpicture}[scale=0.8]
        ...
        \node[above left] at (0, 4) {\footnotesize $\boldsymbol{#2\backslash #3}$};
        \foreach \val [count=\i from 0] in {#4} {
            \pgfmathtruncatemacro{\col}{mod(\i,4)}
...
\tikzset{
    ...
    % İşlemci birimi (veri yolu bileşeni)
    birim/.style={
